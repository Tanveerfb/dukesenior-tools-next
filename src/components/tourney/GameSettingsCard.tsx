...
export default function GameSettingsCard({ roundId }: Props) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`/api/phasmoTourney5/rounds/${roundId}/settings`, { cache: "no-cache" });
        if (res.ok) {
    ...
    <Card className="mb-3">
      ...
      <Card.Body>
        {loading ? (
          ...
          <Tabs defaultActiveKey="player" id="game-settings-tabs" className="mb-3">
            <Tab eventKey="player" title="Player">
              <ul className="mb-3">
                <li>Starting sanity: {s.player.startingSanity}</li>
                <li>Sanity Pill restoration (%): {s.player.sanityPillRestorationPercent}</li>
                <li>Sanity drain speed (%): {s.player.sanityDrainSpeedPercent}</li>
                <li>Sprinting: {s.player.sprinting}</li>
                <li>Player speed (%): {s.player.playerSpeedPercent}</li>
                <li>Flashlights: {s.player.flashlights}</li>
                <li>Lose items and consumables: {s.player.loseItemsAndConsumables}</li>
              </ul>
            </Tab>
            <Tab eventKey="ghost" title="Ghost">
              <ul className="mb-3">
                ...
                <li>Changing favourite room: {s.ghost.changingFavouriteRoom}</li>
                ...
                <li>Fingerprint chance (%): {s.ghost.fingerprintChancePercent}</li>
                <li>Fingerprint duration (s): {s.ghost.fingerprintDurationSeconds}</li>
              </ul>
            </Tab>
            <Tab eventKey="contract" title="Contract">
              <ul className="mb-3">
                ...
                <li>Number of hiding places: {s.contract.numberOfHidingPlaces}</li>
                <li>Sanity monitor: {s.contract.sanityMonitor}</li>
                <li>Activity monitor: {s.contract.activityMonitor}</li>
                <li>Fuse box at start of contract: {s.contract.fuseBoxAtStartOfContract}</li>
                <li>Fuse box visible on map: {s.contract.fuseBoxVisibleOnMap}</li>

...
export async function addTourney4Player(params: {
  name: string;
  discord: string;
  twitch: string;
  prestige: string;
...
export async function getPhasmoTourney4Data() {
  const qcol = query(
    collection(db, TOURNEY4_RECORDS),
...
async function processWinner(
  ...
  await setDoc(
    ref,
    {
      ...
      matchHistory: arrayUnion(mh),
      totalScore: increment(marks),
      points: increment(2),
      matchesPlayer: increment(1),
    },
    { merge: true }
  );
}
async function processLoser(
  playername: string,
  marks: number,
  timestamp: number
) {
...
export async function setMatchRunIDs(
  matchID: string,
  player1runID: string,
  player2RunID: string
) {
  ...
  const player1score = run1.Marks;
  const player2score = run2.Marks;
  let player1 = run1.Participant;
  let player2 = run2.Participant;
  let winner = "";
  let loser = "";
  let tied = false;
  if (player1score > player2score) {
    winner = player1;
    loser = player2;
    await processWinner(winner, player1score, timestamp);
    await processLoser(loser, player2score, timestamp);
  } else if (player1score === player2score) {
    tied = true;
    await processTied(player1, player2, player1score, timestamp);
  } else {
    winner = player2;
    loser = player1;
    await processWinner(winner, player2score, timestamp);
    await processLoser(loser, player1score, timestamp);
  }
  await setDoc(
    matchRef,
    {
...
export async function getAvgRuns() {
  ...
  snap.forEach((docSnap) => {
    ...
    const avgScore =
      player.matchesPlayer > 0 ? player.totalScore / player.matchesPlayer : 0;
...
// ---- Pure helper utilities (extracted for unit tests) ----
...
export function computeTopAveragePlayers(players: RawPlayerStats[], topN = 5) {
  return players
    .map((p) => ({
      ...
      avgScore:
        p.matchesPlayer > 0
          ? Number((p.totalScore / p.matchesPlayer).toFixed(2))
          : 0,
    }))

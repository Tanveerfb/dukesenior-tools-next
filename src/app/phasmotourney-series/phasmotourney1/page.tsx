...
import { Alert, Button, ButtonGroup, Container, Form, FormGroup, InputGroup, Table } from "react-bootstrap";
...
export default function PhasmoTourney1FormPage() {
  ...
  async function calculate(e: React.FormEvent) {
    ...
    if (stars.current?.value === "30 stars (Criteria met)") m += 5; else if (stars.current?.value === "20-29 stars or criteria not met") m += 2; else if (stars.current?.value === "Less than 20 stars") m += 1;
    m += survived ? 5 : -2;
    if (correctGhost) m += 5;
    setMarks(m);
    ...
    if (password.current?.value === "1234") {
      ...
      setError("{Officer access only}. Type in the correct password and try again.");
    }
  }
  function reset() {
    setMarks(0); setSurvived(true); setCorrectGhost(true); setResultScreen(false); setError("");
  }

  return (
    <Container fluid="md" className="mb-3">
      {error && <Alert variant="danger" className="m-0 text-black text-center">{error}</Alert>}
      {resultScreen ? (
        ...
        <Form className="p-3 m-auto" onSubmit={calculate}>
          <Alert variant="primary" className="text-center">Phasmo Tourney #1 (2024)</Alert>
          ...
          <Table hover size="sm" responsive="md">
            <tbody>
              <tr>
                ...
                <td><Form.Check type="switch" label="Cursed item use" ref={curseditemuse} /></td>
              </tr>
              <tr>
          ...
          <FormGroup>
            ...
            <Form.Select className="my-1" aria-label="photos-stars" ref={stars} required>
              <option>Total stars for pictures</option>
              <option value="30 stars (Criteria met)">30 stars (Must have ghost and bone picture)</option>
              <option value="20-29 stars or criteria not met">20-29 stars (select if ghost/bone picture not taken)</option>
              <option value="Less than 20 stars">Less than 20 stars</option>
            </Form.Select>
          </FormGroup>
          <FormGroup className="mb-1">
            <ButtonGroup className="d-flex">
              <Button variant={survived ? "dendro" : "outline-dendro"} disabled={survived} onClick={toggleSurvival} className="m-1">Survived</Button>
              <Button variant={!survived ? "hydro" : "outline-hydro"} disabled={!survived} onClick={toggleSurvival} className="m-1">Died</Button>
            </ButtonGroup>
          </FormGroup>
          <FormGroup className="mb-1">
            <ButtonGroup className="d-flex">
              <Button variant={correctGhost ? "dendro" : "outline-dendro"} disabled={correctGhost} onClick={toggleCorrectGhost} className="m-1">Correct ghost type</Button>
              <Button variant={!correctGhost ? "hydro" : "outline-hydro"} disabled={!correctGhost} onClick={toggleCorrectGhost} className="m-1">Incorrect type</Button>
            </ButtonGroup>
          </FormGroup>

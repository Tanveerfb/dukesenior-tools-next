...
export default async function Page(props: any) {
  ...
  // if the admin SDK isn't installed or initialized we can't hit Firestore.
  // compute this lazily each request so that dynamic imports will set the
  // value when the SDK becomes available.
    ...
    return (
      <div className="container py-4">
        ...
        <p className="text-danger">
          Fire‑and‑forget admin features are disabled (firebase-admin not
          installed)
        </p>
      </div>
    );
  ...
  if (id) {
    ...
    try {
      ...
      if (snap.exists)
        results.push({ type: "doc(users/{id})", data: snap.data() });
    } catch {
    ...
    try {
      ...
      s.forEach((d: any) =>
        results.push({ type: "query: uid field", data: d.data() }),
      );
    } catch {
      // ignore query errors
    }
    ...
    try {
      const s2 = await adminDb
        ...
        .get();
      s2.forEach((d: any) =>
        results.push({ type: "query: Email field", data: d.data() }),
      );
    } catch {
      // ignore query errors
    }
    ...
    try {
      const mapSnap = await adminDb
        ...
        .get();
      if (mapSnap.exists)
        results.push({ type: "usernames mapping", data: mapSnap.data() });
    } catch {
  ...
  return (
    <div className="container py-4">
      ...
      <p className="text-muted">
        Enter an identifier (username, UID, or legacy email) as{" "}
        <code>?id=...</code>
      </p>
      <div className="mb-3">
        <Link href="/admin/debug/users?id=BFZ1ty8xTIYNnjQRAXFtOyl5iG42">
          Test Dukesenior UID
        </Link>
      </div>
      {id ? (

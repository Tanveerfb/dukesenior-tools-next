...
export default function Tourney4StandingsPage(){
          ...
          <Tabs defaultActiveKey="1" className="mb-3 p-2" fill>
            <Tab eventKey="1" title="Bracket 1">
              <Table responsive striped hover>
                ...
                <tbody>
                  {b1.map((b,idx)=>{ const mh = b[0].matchHistory || []; return (
                    <tr key={idx}>
                      <td>{idx+1}</td><td>{b[0].name}</td><td>{b[0].wins}</td><td>{b[0].losses}</td><td>{b[0].ties}</td><td className="fw-bold">{b[0].points}</td>
                      <td className="d-flex flex-row-reverse justify-content-center align-items-center">
                        <span className="mx-1">{mh[mh.length-3]?.substring(0,1) || 'N'}</span>
                        <span className="mx-1">{mh[mh.length-2]?.substring(0,1) || 'N'}</span>
                        <span className="mx-1">{mh[mh.length-1]?.substring(0,1) || 'N'}</span>
                      </td>
                    </tr>); })}
                </tbody>
              </Table>
            ...
            <Tab eventKey="2" title="Bracket 2">
              <Table responsive striped hover>
                <thead><tr><th>Rank</th><th>Name</th><th>W</th><th>L</th><th>T</th><th>Points</th><th>Last 3 matches</th></tr></thead>
                <tbody>
                  {b2.map((b,idx)=>{ const mh = b[0].matchHistory || []; return (
                    <tr key={idx}>
                      <td>{idx+1}</td><td>{b[0].name}</td><td>{b[0].wins}</td><td>{b[0].losses}</td><td>{b[0].ties}</td><td className="fw-bold">{b[0].points}</td>
                      <td className="d-flex flex-row-reverse justify-content-center align-items-center">
                        <span className="mx-1">{mh[mh.length-3]?.substring(0,1) || 'N'}</span>
